		...
		(file: File | FileMetadata): string | null => {
			if (file instanceof File) {
				if (file.size > maxSize) {
					return `O arquivo "${file.name}" é maior do que o tamanho máximo permitido de ${formatBytes(maxSize)}.`;
				}
			} else {
				if (file.size > maxSize) {
					return `O arquivo "${file.name}" é maior do que o tamanho máximo permitido de ${formatBytes(maxSize)}.`;
			...
			if (accept !== '*') {
				...
				if (!isAccepted) {
					return `O arquivo "${file instanceof File ? file.name : file.name}" não é um tipo de arquivo permitido.`;
				}
			}
		...
		(newFiles: FileList | File[]) => {
			...
			newFilesArray.forEach((file) => {
				...
				if (file.size > maxSize) {
					errors.push(
						multiple
							? `Alguns arquivos excedem o tamanho máximo permitido de ${formatBytes(maxSize)}.`
							: `O arquivo excede o tamanho máximo permitido de ${formatBytes(maxSize)}.`,
					);
					return;
				}
