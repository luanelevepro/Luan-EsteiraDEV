...
import { createTiposProduto, deleteTiposProduto, getTiposProduto, updateTiposProduto } from '@/services/api/sistema';
...
const columns: ColumnDef<TipoProdutoData>[] = [
	{
...
export default function CadastroTipoProdutoPage() {
	...
	if (isError) {
		toast.error(error?.message || 'Erro ao carregar os tipos de produto.');
	}

	return (
		<>
			<Head>
				<title>Cadastro de Tipos de Produto | Esteira</title>
			</Head>
			<DashboardLayout title='Tipos de Produto' description='Gerencie o cadastro de tipos de produto.'>
...
function HandleInsert({ children, data }: { children: React.ReactNode; data?: TipoProdutoData }) {
	const queryClient = useQueryClient();
	const [open, setOpen] = useState(false);
	const [isLoading, setIsLoading] = useState(false);
	const [info, setInfo] = useState<TipoProdutoData>(data ?? { ds_descricao: '', ds_codigo: '' });
	...
	async function sendInformation() {
		...
		try {
			if (data?.id) {
				await updateTiposProduto(data?.id, info);
			} else {
				await createTiposProduto(info);
			}
			await queryClient.invalidateQueries({ queryKey: ['get-tipos-produtos'] });
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{data?.id ? 'Editar' : 'Adicionar'} tipos de produto</DialogTitle>
...
function HandleDelete({ data }: { data: TipoProdutoData }) {
	const [open, setOpen] = useState(false);
	const queryClient = useQueryClient();
	...
	async function DeleteData() {
		...
		try {
			await deleteTiposProduto(data?.id);
			await queryClient.invalidateQueries({ queryKey: ['get-tipos-produtos'] });
