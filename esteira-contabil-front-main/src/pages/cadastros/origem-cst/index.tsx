...
import { createOrigemCST, deleteOrigemCST, getOrigemCST, updateOrigemCST } from '@/services/api/sistema';

export interface OrigemCSTData {
	id?: string;
	ds_descricao: string;
	ds_codigo: string;
}

const columns: ColumnDef<OrigemCSTData>[] = [
	{
...
export default function CadastroOrigemCSTPage() {
	...
	return (
		<>
			<Head>
				<title>Cadastro de Origem CST | Esteira</title>
			</Head>
			<DashboardLayout title='Origem CST' description='Gerencie o cadastro de origem CST.'>
...
function HandleInsert({ children, data }: { children: React.ReactNode; data?: OrigemCSTData }) {
	const queryClient = useQueryClient();
	const [open, setOpen] = useState(false);
	const [isLoading, setIsLoading] = useState(false);
	const [info, setInfo] = useState<OrigemCSTData>(data ?? { ds_descricao: '', ds_codigo: '' });
	...
	async function sendInformation() {
		...
		try {
			if (data?.id) {
				await updateOrigemCST(data?.id, info);
			} else {
				await createOrigemCST(info);
			}
			await queryClient.invalidateQueries({ queryKey: ['get-origem-cst'] });
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{data?.id ? 'Editar' : 'Adicionar'} Origem CST</DialogTitle>
...
function HandleDelete({ data }: { data: OrigemCSTData }) {
	const [open, setOpen] = useState(false);
	const queryClient = useQueryClient();
