...
import { createCST, deleteCST, getCST, updateCST } from '@/services/api/sistema';
...
const columns: ColumnDef<CSTData>[] = [
	{
...
export default function CadastroCSTPage() {
	...
	return (
		<>
			<Head>
				<title>Cadastro de CST | Esteira</title>
			</Head>
			<DashboardLayout title='CST' description='Gerencie o cadastro de CST.'>
...
function HandleInsert({ children, data }: { children: React.ReactNode; data?: CSTData }) {
	const queryClient = useQueryClient();
	const [open, setOpen] = useState(false);
	const [isLoading, setIsLoading] = useState(false);
	const [info, setInfo] = useState<CSTData>(data ?? { ds_descricao: '', ds_codigo: '' });
	...
	async function sendInformation() {
		...
		try {
			if (data?.id) {
				await updateCST(data?.id, info);
			} else {
				await createCST(info);
			}
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{data?.id ? 'Editar' : 'Adicionar'} CST</DialogTitle>
					<DialogDescription>Insira os dados para continuar.</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit} className='grid gap-4'>
					<div className='grid grid-cols-3 gap-4'>
						<div className='col-span-1 grid gap-2'>
							<Label htmlFor='name'>Código tributação</Label>
...
function HandleDelete({ data }: { data: CSTData }) {
	const [open, setOpen] = useState(false);
	const queryClient = useQueryClient();
