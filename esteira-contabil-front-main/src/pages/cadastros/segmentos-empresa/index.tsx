...
import { Info, Pencil, Plus, RefreshCw, SearchIcon, Trash } from 'lucide-react';
...
import { createSegmentosEmpresas, deleteSegmentosEmpresas, getSegmentosEmpresas, updateSegmentosEmpresas } from '@/services/api/fiscal';
import { useCompanyContext } from '@/context/company-context';

export interface SegmentosEmpresasData {
	id?: string;
	ds_descricao: string;
	is_padrao?: boolean;
}

const columns: ColumnDef<SegmentosEmpresasData>[] = [
	{
...
export default function CadastroSegmentosEmpresaPage() {
	...
	return (
		<>
			<Head>
				<title>Cadastro de Segmentos Empresa | Esteira</title>
			</Head>
			<DashboardLayout title='Segmentos Empresa' description='Gerencie o cadastro de Segmentos Empresa.'>
...
function HandleInsert({ children, data }: { children: React.ReactNode; data?: SegmentosEmpresasData }) {
	const queryClient = useQueryClient();
	const [open, setOpen] = useState(false);
	const [isLoading, setIsLoading] = useState(false);
	const { state } = useCompanyContext();
	const [info, setInfo] = useState<SegmentosEmpresasData>(data ?? { ds_descricao: '' });
	...
	async function sendInformation() {
		...
		try {
			if (data?.id) {
				await updateSegmentosEmpresas(data?.id, info);
			} else {
				await createSegmentosEmpresas(info);
			}
			await queryClient.invalidateQueries({ queryKey: ['get-segmentos-empresa'] });
			await queryClient.invalidateQueries({ queryKey: ['empresa-regime', state] });
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{data?.id ? 'Editar' : 'Adicionar'} Segmentos Empresa</DialogTitle>
...
function HandleDelete({ data }: { data: SegmentosEmpresasData }) {
	const [open, setOpen] = useState(false);
	const queryClient = useQueryClient();
	...
	async function DeleteData() {
		...
		try {
			await deleteSegmentosEmpresas(data?.id);
			await queryClient.invalidateQueries({ queryKey: ['get-segmentos-empresa'] });
