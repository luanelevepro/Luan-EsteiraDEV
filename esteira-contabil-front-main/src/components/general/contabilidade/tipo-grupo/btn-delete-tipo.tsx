...
import { deleteTipoGrupo } from '@/services/api/tipo-grupo';
...
export default function HandleDelete({ tipo }: HandleDeleteProps) {
	...
	async function DeleteData() {
		...
		if (tipo.js_con_grupo_contas.length > 0) {
			toast.error('Existem Grupos Vinculados.');
			setLoading(false);
			setOpen(false);
			return;
		}
		try {
			await deleteTipoGrupo(tipo?.id);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>Isso deletar o registro, essa ação é irreversível.</DialogDescription>
				</DialogHeader>
				<DialogFooter>
					<DialogClose asChild>
